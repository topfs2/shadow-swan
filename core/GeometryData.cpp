...
GeometryData::GeometryData(const Geometry::varray &vertices) : m_vertices(vertices) {
    for(int i = 0; i < vertices.size(); i++) {
        m_indices.push_back(i);
    }
}

GeometryData::GeometryData(const Geometry::varray &vertices, const Geometry::iarray &indices) : m_vertices(vertices), m_indices(indices) { }
...
void GeometryData::vertex(size_t index, glm::vec3 position, glm::vec3 normal, glm::vec2 uv, glm::vec3 tangent, glm::vec3 bitangent) {
    vertex(index, Geometry::vertex_t(position, normal, uv, tangent, bitangent));
}

void GeometryData::vertex(size_t index, glm::vec3 position, glm::vec3 normal, glm::vec2 uv) {
    vertex(index, Geometry::vertex_t(position, normal, uv));
}
...
void GeometryData::vertex(size_t index, Geometry::vertex_t v) {
    m_vertices[index] = v;
}

Geometry::vertex_t GeometryData::vertex(size_t index) const {
    return m_vertices[index];
}

void GeometryData::indices(const Geometry::iarray &indices) {
    m_indices = indices;
}

Geometry::iarray GeometryData::indices() const {
    return m_indices;
}
...
void GeometryData::vertices(const Geometry::varray &vertices) {
    m_vertices = vertices;
}

Geometry::varray GeometryData::vertices() const {
    return m_vertices;
}

const Geometry::vertex_t *GeometryData::verticesRaw() const {
...
void GeometryData::computeNormals() {
    ...
    for (unsigned int i = 0; i < m_indices.size(); i += 3) {
        ...
        auto &a = m_vertices[i0];
        auto &b = m_vertices[i1];
        auto &c = m_vertices[i2];
